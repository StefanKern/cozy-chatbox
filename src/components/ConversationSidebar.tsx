...
const ConversationSidebar = ({ currentSessionId, onSelectSession, onNewChat }: ConversationSidebarProps) => {
  ...
  useEffect(() => {
    const fetchConversations = async () => {
      ...
      // Group by session_id and get first user message
  ...
  return (
    <div
      className={cn(
        "h-screen bg-chatbg border-r border-accent transition-all duration-300",
    ...
    >
      ...
      <div className="p-2">
        <Button
          variant="secondary"
          className={cn("w-full mb-4", isCollapsed && "p-2")}
          onClick={onNewChat}
        >
